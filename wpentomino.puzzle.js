...
// global variable
//==================
var SCREEN_BOARD_X;		//	ordenada X de tablero
...
{
	if (DEBUG) { console.log("linea 189: function playPuzzle(newPuzzle) \n" +
		"newPuzzle: " + newPuzzle	); };
	waitIdleDemo();
...
// initial screen position and background color 
{
	...
	console.log( "demo      --> top:" + (Math.floor(SCREEN_Y/2) - 20) + "px; left:" + (2) + "px");
	*/
...
function createPuzzle(newPuzzle, activePoly)
{
	...
	if (0)	//	(DEBUG)
	{
		...
		var fixedBlock = (boardSizeInfo[gBoardSizeId].numOfLevel - gLevelId)* 2;		//	para produccion
	}

	if (DEBUG)
	{
		console.log("lin 422, gBoardSizeId: " + gBoardSizeId );
		console.log("lin 423, fixedBlock: " + fixedBlock  );
	} 
...
{
	console.log("linea 437: inicializando el tablero");
	console.log("linea 438: numOfFixedBlocks: " + numOfFixedBlocks);
	...
	if (DEBUG) { console.log("Linea 504: Se agregaron elementos al gStage");};
...
function randomPolyInitPos(availablePoly)
{
	...
	{
		console.log(
			"(availablePoly > 5)?Math.floor((availablePoly+1)/2): availablePoly; \n" + 
			" - (availablePoly > 5): " + (availablePoly > 5) + "\n" +
			" - Math.floor((availablePoly+1)/2): " + Math.floor((availablePoly+1)/2) + " \n" +
			" - availablePoly: " + availablePoly + " \n - " +
			(availablePoly > 5)?Math.floor((availablePoly+1)/2): availablePoly
			)
	}
...
function addFixedBlock2Layer(op, numOfFixedBlocks)
{
	...
	for(var id=0; id < gBlockGroup.length; id++) {
		...
		if(gBlockGroup[id].order >= 0 && gBlockGroup[id].order < numOfFixedBlocks) {
			...
			/*
			console.log(" Ingresando a addFixedBlock2Layer(op, numOfFixedBlocks)" );
			console.log( block );
			console.log( block [0] );
			console.log( block [1] );
			console.log( block [2] );
...
//	convierte sistema de coordenadas de bloque a coordenadas de poligono
//	con estas ultimas se puede dibujar en la pantalla
//-----------------------------------------------------------------
function block2Polygon(block)
{
	/*
	console.log(" Ingresando a block2Polygon(block)" );
	console.log( block [0] );
	console.log( block [1] );
	console.log( block [2] );
	console.log( block [3] );
